...
prompt = """
You will be provided with a dictionary, and your job is to "tag" the location dictionary with as many of the following tags that apply (based on the group of people you can visit these places with):
Friends, Family, Romantic Partner, Business Partner
You need to output an array, with the filters. DO NOT CREATE YOUR OWN FILTERS, STICK TO THE LIST PROVIDED. 
DO NOT USE FORMATTING SUCH AS ```json AND OTHERS. ONLY OUTPUT THE FILTERS. Example:
Input:
...
f = open('static/data/attraction-people.txt', 'a')
